...
> 它会像带菜单的安装脚本一样工作：**静默**读索引 → 第一条回复只弹菜单 → 等你输编号 → 固定四关执行 → 回到菜单。
>
> 私有仓用户请把第一段里的仓名换成 `kunpeng-router-tuning`。
> 想加功能：在菜单加一行、在「功能与依据」补一行、在硬约束第 1 条核对排除项，其余结构不用动。
...
1. **首输出必须逐字只有菜单**——不加「静默 + 逐字照抄 + 禁止清单」的话，AI 会先客套一句「好的，我正在读取仓库」，菜单仪式感全毁，还可能顺手往下跑。
2. **菜单必须带任务 id**——只写「1) 装 OpenClash」的话，AI 不知道该读哪个任务包，会自由发挥；playbook 路径放「功能与依据」段，菜单行保持干净。
3. **输入语法要穷举**——单选、多选顺序、退出、非法输入都写死，否则 AI 自己发明规则。
4. **范围边界写死到 id 级**——store.* 列出全部 4 个具体 id，加上 AdGuard / NAS / Portainer 汉化；不然 AI 看到 index.json 里还有 30 多个任务，很容易顺手多装。
