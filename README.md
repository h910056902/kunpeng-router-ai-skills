...
| **T5 · 第三方 NROS 插件安装器（maye 助手）· 总入口** | [`tasks/05-nros-plugin-installer.md`](tasks/05-nros-plugin-installer.md) | `offline/maye/ssh-nradio-plugin-installer-lite.sh`（**精简版**：红线功能已物理删除，60,590 行；**真机已验证** —— 菜单走查 + 只读体检 handler 端到端（0 文件净变动）+ 安装类 handler 端到端（ttyd，19 文件/19 目录被改）；副作用一律用 `-nt` 法复核，`find -newer` 在本固件会假阴性）+ `PROVENANCE.md` / `SHA256SUMS`；适配器 `scripts/adapt_maye_assistant.py` |
